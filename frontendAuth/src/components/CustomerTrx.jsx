...
import { getAllTransactions, getHistoryTrxCustomerId, orderService, updateTransaction } from "../services/TransactionsService";
import { addTransactions, setTransactions, updateTrxById } from "../store/reducers/transactionsSlice";
...
export function CustomerTrx() {
  ...
  console.log("userlogiiin"+userLogin.id)
  useEffect(()=> {
    ...
    useEffect(() => {
      const fetchTransactions = async () => {
        try {
          ...
          const response = await getHistoryTrxCustomerId(userLogin.id, pageSize, currentPage);
          setTotalPages(response.data.paging.totalPage)
          console.log("totalPage"+response.data.paging.totalPage)
...
//   );
  ...
  const handleOrder = async(e) => {
    e.preventDefault()
    try{
        const response = await orderService(order)
      ...
      <Dialog open={open} size="xs" handler={handleOpen} onSubmit={updateTrx}>
        ...
        <DialogBody>
          <Typography className="mb-10 -mt-7 " color="gray" variant="lead">
            Choose service laundry and then click button.
          </Typography>
          <div className="grid gap-6">
          <Typography variant="h6" color="blue-gray">
            Select Status
            </Typography>
            <Input value={userLogin.id} disabled />
            <Select color="teal" label="update status" name='statusId' onChange={(e) => setOrder({ ...order, serviceId: e })}>
        ...
        <DialogFooter className="space-x-2">
          ...
          <Button type="submit" variant="gradient" color="gray" onClick={handleOrder} >
            Order
          </Button>
        </DialogFooter>
      </Dialog>
